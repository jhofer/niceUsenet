...
module.exports.parseMovie = function parseMovie(html) {
  ...
  var thxLink = $('.postfoot a.post_thanks_button').attr('href');
  var imdbLink = "";
  ...
  var movie = {
    thxLink: 'http://www.usenetrevolution.info/vb/' + thxLink,
    imdbLink: imdbLink
  };
...
module.exports.parseDownloadInfos = function (html) {

  var $ = cheerio.load(html);
  var infos = {};
  infos.downloadLink = $('.unhiddencontentbox table a').attr('href');
  infos.password = $('.unhiddencontentbox font[size="4"] font').next().text().replace("\n",'');
